...
export class TEACHERComponent implements OnInit {
  ...
  extractLabsOfThisStudent() {
    ...
    let arrayOf_LabJoinCodesOfJoinedLabs: string[] = this.TheTeacher[0].LabJoinCodesOfJoinedLabs;
      ...
      for (let i = 0; i < this.AllLabsRecieved.length; i++) {
        for (let j = 0; j < arrayOf_LabJoinCodesOfJoinedLabs.length; j++) {
          if (this.AllLabsRecieved[i]._id == arrayOf_LabJoinCodesOfJoinedLabs[j]) {
            this.LabsOfThisTeacher.push(this.AllLabsRecieved[i]);
  ...
  onVisitLabButtonClicked(labId: string, modalButtonReferrence: HTMLButtonElement) {
    ...
    window.location.href = "/TEACHER/Lab";
  ...
  onJoinLabButtonClicked(enteredJoinCode: string, modalButtonReferrence: HTMLButtonElement) {
    ...
    for (let k = 0; k < this.TheTeacher[0].LabJoinCodesOfJoinedLabs.length; k++) {
      if (this.TheTeacher[0].LabJoinCodesOfJoinedLabs[k] == enteredJoinCode) {
    ...
    console.log('003.1 this.TheStudent[0].LabJoinCodesOfAppliedLabs.length ==>', this.TheTeacher[0].LabJoinCodesOfAppliedLabs.length);
    console.log('003.2 this.TheStudent[0].LabJoinCodesOfJoinedLabs.length ==>', this.TheTeacher[0].LabJoinCodesOfJoinedLabs.length);
    for (let p = 0; p < this.TheTeacher[0].LabJoinCodesOfAppliedLabs.length; p++) {
      console.log('<===================> ');
      console.log(p + 1);
      console.log('<===================> ');
      arrayOf_LabJoinCodesOfAppliedLabs.push(this.TheTeacher[0].LabJoinCodesOfAppliedLabs[p]);
    }
    ...
    arrayOf_LabJoinCodesOfAppliedLabs = [...this.TheTeacher[0].LabJoinCodesOfAppliedLabs];
    console.log('004 this.TheStudent[0]', this.TheTeacher[0]);
    console.log('004.1 this.TheStudent[0].LabJoinCodesOfAppliedLabs', this.TheTeacher[0].LabJoinCodesOfAppliedLabs);
    ...
    if (checkIf_enteredJoinCode_MatchesWithAnyLab) {
      ...
      if (checkIf_enteredJoinCode_IsEnteredAgain) {
        ...
        this.TheTeacher[0].LabJoinCodesOfAppliedLabs.push(enteredJoinCode);
        console.log('this.TheStudent[0] before updateing', this.TheTeacher[0]);
        this.usersService.updateThisUser(this.TheTeacher[0], this.TheTeacher[0]._id);
